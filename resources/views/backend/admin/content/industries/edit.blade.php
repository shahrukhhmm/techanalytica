...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="fw-bold">Edit Industry</h4>
      ...
      <form action="{{ route('admin.industries.update', $industry) }}" method="POST">
        ...
        <div class="mb-3">
          ...
          <textarea class="form-control @error('description') is-invalid @enderror" 
                    id="description" name="description" rows="3">{{ old('description', $industry->description) }}</textarea>
          @error('description')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
          ...
          <select class="form-select @error('suggested_by_vendor_id') is-invalid @enderror" 
            ...
            @foreach($vendors as $vendor)
              <option value="{{ $vendor->id }}" 
                      {{ old('suggested_by_vendor_id', $industry->suggested_by_vendor_id) == $vendor->id ? 'selected' : '' }}>
